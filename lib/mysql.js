...
MySQL.prototype._modifyOrCreate = function(model, data, options, fields, cb) {
  ...
  for (var i = 0, n = fields.names.length; i < n; i++) {
    if (!fields.properties[i].id) {
      setValues.push(
        new ParameterizedSQL(fields.names[i] + '=' + columnValues[i].sql, columnValues[i].params)
      );
    }
  }
 ...
 * @param {Function} [cb] The callback function
 */
MySQL.prototype.save = MySQL.prototype.updateOrCreate = function(model, data, options, cb) {
  var fields = this.buildFields(model, data);
  this._modifyOrCreate(model, data, options, fields, cb);
};

MySQL.prototype.getInsertedId = function(model, info) {
  var insertedId = info && typeof info.insertId === 'number' ? info.insertId : undefined;
  return insertedId;
};
...
MySQL.prototype.getCountForAffectedRows = function(model, info) {
  var affectedRows = info && typeof info.affectedRows === 'number' ? info.affectedRows : undefined;
  return affectedRows;
};
...
MySQL.prototype.buildExpression = function(
  columnName,
  operator,
  operatorValue,
  propertyDefinition
...
MySQL.prototype.findForUpdate = function(model, id, options, cb) {
  if (id == null) {
    process.nextTick(function() {
  ...
  return this.allForUpdate(model, filter, options, function(err, results) {
    cb(err, (results && results[0]) || null);
  });
};

MySQL.prototype.allForUpdate = function(model, filter, options, cb) {
  var self = this;
  // Order by id if no order is specified
  filter = filter || {};
  var stmt = this.buildSelectForUpdate(model, filter, options);
  this.execute(stmt.sql, stmt.params, options, function(err, data) {
    ...
    if (filter && filter.include) {
      self.getModelDefinition(model).model.include(objs, filter.include, options, cb);
    } else {
      cb(null, objs);
    }
  });
...
MySQL.prototype.buildSelectForUpdate = function(model, filter, options) {
  if (!filter.order) {
